...
import copy
def highlight_qaresult(qa_result):
    ...
    if qa_result == []:
        print('API broken')
        return 1
    qaresult = copy.deepcopy(qa_result)
    ## tokenize query
            ...
            if len(word_tokens) > 1:
                for j in word_tokens:
                    if j.lower() in query_tokens:
                        context_highlight = context_highlight + "<b>" + j + "</b>"
                    else:
                        context_highlight = context_highlight + j
                ...
                for j in word_tokens:
                    if j.lower() in query_tokens:
                        ...
                        context_highlight = context_highlight + j + " "
        qaresult[i]['context'] = context_highlight
    return qaresult
